...
class Config:
    ...
    # log_path = 'save.txt'
    log_path = Path('~/.shrec/').expanduser()
    log_path.mkdir(parents=True, exist_ok=True)
    log_path = str(log_path / 'save.txt')

    objs_folder = Path(argv[0])
    # phase = argv[1]
...
class ShrecDataset(object):
    def __init__(self, config):
        super().__init__()
        self.cfg = config
        self.root = config.objs_folder
        self.output_path = self.root / '..' / config.output_name
        self.output_path.mkdir(parents=True, exist_ok=True)
        self.list = []
        self.list.extend(self.root.glob('*.obj'))
        self.list.extend(self.root.glob('*.OBJ'))
        self.list.sort()

    def __getitem__(self, index):
        obj_path = self.list[index]
        return (obj_path)
    ...
    def export_2d(self):
        ...
        for id in range(start, end):
            if id >= len(self):
                ...
                break
            obj_path = self[id]
            output_name = self[id].stem

            for k in range(-3, 4):
                z = math.sin(math.pi/6*k) * 1  # 1 Unit
                y = math.cos(math.pi/6*k) * 1  # 1 Unit
                out = str(self.output_path / 'ring{}'.format(k+3) / output_name).strip()
                take_a_snap(str(obj_path), out, (0, y, z))
            self.log(id+1)
